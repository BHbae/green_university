package university.green.staff.repository;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.ResultSet;

import university.green.staff.model.SubPeriodDTO;
import university.green.staff.repository.interfaces.SubPeriodRepository;
import university.green.util.DBUtil;

public class SubPeriodRepositoryImpl implements SubPeriodRepository{

	// 수강신청 기간 - 조회
	@Override
	public SubPeriodDTO getSubPeriod() {
		SubPeriodDTO subPeriodStatus = new SubPeriodDTO();
		final String GET_STATUS=" select * from sub_period_tb ";
		try (Connection conn=DBUtil.getConnection();
				PreparedStatement pstmt=conn.prepareStatement(GET_STATUS)){
			ResultSet rs=pstmt.executeQuery();
			if(rs.next()) {
				subPeriodStatus=SubPeriodDTO.builder().id(rs.getInt("id")).status(rs.getInt("status")).build();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		return subPeriodStatus;
	}

	// 수강신청 기간 - 시작
	@Override
	public void startSubPeriod() {
		final String START_PERIOD=" update sub_period_tb set status = 1 ";
		try (Connection conn=DBUtil.getConnection()){
			conn.setAutoCommit(false);
			try (PreparedStatement pstmt=conn.prepareStatement(START_PERIOD)){
				pstmt.executeUpdate();
				conn.commit();
			} catch (Exception e) {
				conn.rollback();
				e.printStackTrace();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	// 수강신청 기간 - 종료
	@Override
	public void stopSubPeriod() {
		final String START_PERIOD=" update sub_period_tb set status = 2 ";
		try (Connection conn=DBUtil.getConnection()){
			conn.setAutoCommit(false);
			try (PreparedStatement pstmt=conn.prepareStatement(START_PERIOD)){
				pstmt.executeUpdate();
				conn.commit();
			} catch (Exception e) {
				conn.rollback();
				e.printStackTrace();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

}
